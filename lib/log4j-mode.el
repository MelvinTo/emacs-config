...
(defun log4cxx-browse-source ()
  "Look up the identifier around or before point, and show the related source"
  (interactive)
  ...
  (save-excursion
    ...
    (let ((end-pos (save-excursion  (move-end-of-line nil)  (point)))
          file
          line)
      ...
      (if (re-search-forward "[^(]*(\\([^:]*\\):\\([^:]*\\))[^)]*" end-pos t)
          (progn
            ...
            ;; TODO: This has to be fixed to support tagging/ipdcs
            (with-project-root (find-file-other-window (concat "src/" file)))
...
(unless log4j-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map [(control c) (control o)] 'log4cxx-browse-source)
